DApp = {
    ...
    development: false,
    //Rinkeby:
    factoryAddress: "0xEc95aB4FBeBBEF42B41434F38A574b5c7aa8C822",
    tokenAddress: "0xAc6F52b8a39Bf1213563F2bfE58FCAE72E0a2D9E",
    ...
    initWeb3: function() {
        window.addEventListener('load', async() => {
            ...
            if (typeof web3 !== 'undefined') {
                DApp.web3Provider = new web3.currentProvider;
            } else {
    ...
    getTestTokenContract: function() {
        if (DApp.development)
            return DApp.testTokenContract.deployed();
        else
            return DApp.testTokenContract.at(DApp.tokenAddress);
    },
    ...
    loadSingleWallet: function(walletAddress) {
        ...
        // Load TestToken wallets.
        DApp.getTestTokenContract()
            .then(function(tokenInstance) {
                return tokenInstance.balanceOf(walletAddress);
            })
            .then(function(info) {
                var amount = info.toNumber();
                DApp.addFundsToWallet(walletAddress, 'testtoken', amount);
            });
    },
                ...
                .then(function(tokenInstance) {
                    ...
                    DApp.walletContract.at(walletAddress)
                        ...
                        .then(function(tx) {
                            ...
                            var amount = withdrawEvent["amount"].toNumber();
                            DApp.addFundsToWallet(walletAddress, 'testtoken', (-1) * amount);
                        });
                })
        }
    },
    ...
    updateClaimForm: function() {
        ...
        if (currency == "ether") {
            ...
            var testValue = DApp.getKnownWalletBallance(wallet, 'testtoken')
            form.find("#claimableAmount").val(testValue);
        } else {
    ...
    valueFormatter: function(cell, row) {
        ...
        if (ethValue == 0 && testValue == 0) {
            return 'Wallet empty';
        }
        var html = '';
        if (ethValue > 0) { html += `${ethValue} Ether</br>` }
        if (testValue > 0) { html += `${testValue} TestToken` }
